...
class MyDataset(QNDataset):
    ...
    def __getitem__(self, index):
        ...
        gt = self.transforms(IMG.fromarray(img_obj.ground_truth))
        return {'indices': self.indices[index], 'input': img_tensor, 'label': gt.squeeze()}

    @property
    def transforms(self):
        return tmf.Compose(
            [tmf.CenterCrop((256, 256)), tmf.ToTensor()])
...
class MyTrainer(QNTrainer):
    ...
    def _init_nn(self):
        self.nn['model'] = MYModel(self.args['num_channel'], self.args['num_class'], r=self.args['model_scale'])
